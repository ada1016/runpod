...
const GOD_MODE_HP_ARGUMENT = new Int64("-9999999999");
...
function attackTarget(): number {
    return Math.min(
        ATTACK_CAP,
        Math.max(1, Math.round(settings.attackMultiplier))
    );
}

function applyAttack(hero: Il2Cpp.Object, target: number): boolean {
...
function processOnce(): TickStats {
    ...
    if (battleUiClass === null || (!settings.godMode && !settings.attackEnabled)) {
        return stats;
    }
    ...
    for (const battleUi of battleUis) {
        ...
        for (const hero of heroes) {
            if (settings.godMode && applyGodMode(hero)) {
                stats.healed++;
            }
            if (settings.attackEnabled && applyAttack(hero, target)) {
                stats.attackChanged++;
            }
        }
    }
...
rpc.exports = {
    ...
    configure(options: Partial<Settings>): object {
        ...
        if (typeof options.attackMultiplier === "number" &&
            Number.isFinite(options.attackMultiplier)) {
            settings.attackMultiplier = Math.max(
                1,
                Math.trunc(options.attackMultiplier)
            );
        }
    ...
    setattack(enabled: boolean, multiplier?: number): object {
        settings.attackEnabled = Boolean(enabled);
        if (typeof multiplier === "number" && Number.isFinite(multiplier)) {
            settings.attackMultiplier = Math.max(1, Math.trunc(multiplier));
        }
        return statusSnapshot();
    },
