...
type Settings = {
    ...
    addReviveNoOp: boolean;
};

type ReviveGuardEntry = {
...
let reviveGuardPending = false;
let reviveGuardNextAttemptAt = 0;
let reviveGuardDiscoveryAttempts = 0;
let reviveGuardTotalRestores = 0;
let reviveTimesFieldOffset: number | null = null;
const reviveGuards = new Map<string, ReviveGuardEntry>();
...
function resolveBattleStatisticsClass(): Il2Cpp.Class | null {
    ...
    try {
        const klass = assembly.image.class(BATTLE_STATISTICS_NAME);
        const field = klass.field(REVIVE_TIMES_FIELD_NAME);
        battleStatisticsClass = klass;
        reviveTimesFieldOffset = field.offset;
        log(
            "resolved " + LEVELS_ASSEMBLY_NAME + "!" +
            BATTLE_STATISTICS_NAME + "." + REVIVE_TIMES_FIELD_NAME +
            " offset=" + field.offset
        );
        return klass;
    } catch (_) {
...
function releaseReviveGuards(): void {
    for (const guard of reviveGuards.values()) {
        try {
            guard.reference.free();
        } catch (_) {
            // The runtime may already have released a dead weak reference.
        }
    }
    reviveGuards.clear();
}

function discoverBattleStatistics(): number {
    const klass = resolveBattleStatisticsClass();
    if (klass === null || reviveTimesFieldOffset === null) {
        return 0;
    }
    ...
    for (const instance of instances) {
        ...
        if (reviveGuards.has(handle)) {
            continue;
        }

        const baseline = instance.handle
            .add(reviveTimesFieldOffset)
            .readS32();
        reviveGuards.set(handle, {
            // A weak handle follows the object without keeping an old battle's
            // statistics object alive indefinitely.
            reference: instance.weakRef(true),
            handle,
            baseline,
...
function applyReviveNoOpGuard(): void {
    if (!settings.addReviveNoOp) {
        return;
    }

    if (reviveGuardPending && Date.now() >= reviveGuardNextAttemptAt) {
        reviveGuardDiscoveryAttempts++;
        reviveGuardNextAttemptAt = Date.now() + REVIVE_GUARD_RETRY_MS;
        try {
            const added = discoverBattleStatistics();
            if (added > 0 || reviveGuards.size > 0) {
                reviveGuardPending = false;
                reviveGuardNextAttemptAt = 0;
                log(
                    "AddRevive no-op guard watching " +
                    reviveGuards.size + " BattleStatistics object(s)"
                );
            }
        } catch (error) {
            logError("discover BattleStatistics", error);
        }
    }

    if (reviveTimesFieldOffset === null) {
        return;
    }

    for (const [handle, guard] of reviveGuards) {
        const instance = guard.reference.target;
        if (instance === null || !isAlive(instance)) {
            try {
                guard.reference.free();
            } catch (_) {
                // Ignore an already released weak handle.
            }
            reviveGuards.delete(handle);
            reviveGuardPending = true;
            reviveGuardNextAttemptAt = 0;
            continue;
        }

        try {
            const address = instance.handle.add(reviveTimesFieldOffset);
            const current = address.readS32();
            guard.lastObserved = current;
            if (current !== guard.baseline) {
                address.writeS32(guard.baseline);
                guard.restores++;
                reviveGuardTotalRestores++;
                log(
                    "suppressed AddRevive effect: ReviveTimes " +
                    current + " -> " + guard.baseline
                );
            }
        } catch (error) {
            logError("BattleStatistics.ReviveTimes guard", error);
        }
    }
}
...
function statusSnapshot(): object {
    return {
        ...
        addReviveNoOp: {
            enabled: settings.addReviveNoOp,
            pending: reviveGuardPending,
            classResolved: battleStatisticsClass !== null,
            fieldOffset: reviveTimesFieldOffset,
            discoveryAttempts: reviveGuardDiscoveryAttempts,
            watchedObjects: reviveGuards.size,
            totalRestores: reviveGuardTotalRestores,
            objects: Array.from(reviveGuards.values()).map(guard => ({
                handle: guard.handle,
                baseline: guard.baseline,
                lastObserved: guard.lastObserved,
...
rpc.exports = {
    ...
    configure(options: Partial<Settings>): object {
        ...
        if (typeof options.addReviveNoOp === "boolean") {
            settings.addReviveNoOp = options.addReviveNoOp;
            reviveGuardPending = settings.addReviveNoOp;
            reviveGuardNextAttemptAt = 0;
            if (!settings.addReviveNoOp) {
                releaseReviveGuards();
            }
    ...
    setaddrevivenoop(enabled: boolean): object {
        settings.addReviveNoOp = Boolean(enabled);
        releaseReviveGuards();
        reviveGuardPending = settings.addReviveNoOp;
        reviveGuardNextAttemptAt = 0;
        return statusSnapshot();
    ...
    refreshreviveguard(): object {
        if (!settings.addReviveNoOp) {
            return {
                error:
                    "AddRevive no-op is disabled; call " +
                    "setaddrevivenoop(true)",
                ...statusSnapshot()
            };
        }
        releaseReviveGuards();
