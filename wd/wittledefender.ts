...
/*
 * Commands: revive(), reroll(96), inc(100), heal(50), expower(1.2), read(),
 * god(), god(5), god(false), creroll(), creroll(10), creroll(false), traceend(),
 * traceend(false), traceendstatus().
 * No gameplay command runs automatically inside this agent. hook.sh reads
 * default_hook from the project's hook_pid.json after loading the compiled
 * JavaScript. This keeps project startup settings outside the TypeScript.
 *
 ...
 * expower(multiplier) multiplies positive EX-weapon energy gains through
 * SWeaponChargeSpeedPct (key 151); it does not hook AddEnergy or affect costs.
 ...
 * creroll() sets active Crusade battles' ReRandomSkillNum to 96;
 * creroll(n) selects another absolute remaining count, and creroll(false)
 * restores the value saved when that active Crusade manager was enrolled.
 * This is a one-shot field mutation: no method hook or polling.
...
const DEFAULT_CRUSADE_REROLL = 96;
const AD_REVIVES = 5;
...
const EX_WEAPON_ENERGY_PERCENT_KEY = 151;
const MAX_EX_POWER_MULTIPLIER = 10;
...
type CrusadeRerollEntry = {
    reference: Il2Cpp.GCHandle;
    original: number;
};
const crusadeRerollEntries = new Map<string, CrusadeRerollEntry>();
...
function crusadeRerollCommand(value: boolean | number = DEFAULT_CRUSADE_REROLL): Promise<object> {
    if (typeof value !== "boolean" &&
        (!Number.isSafeInteger(value) || value < 0 || value > 0x7fffffff)) {
        return Promise.resolve({ ok: false,
            error: "use creroll(), creroll(false), or creroll(n) with a non-negative Int32" });
    }
    const enabled = value !== false;
    const target = typeof value === "number" ? value : DEFAULT_CRUSADE_REROLL;

    return runCommand("creroll", () => {
        const errors: string[] = [];
        ...
        if (enabled) {
            ...
            for (const manager of Il2Cpp.gc.choose(klass)) {
                ...
                const handle = manager.handle.toString();
                try {
                    const mode = objectField(manager, "BattleMode");
                    const startData = objectField(manager, "StartData");
                    const attr = objectField(manager, "InGameAttr");
                    const battleType = startData === null ? null
                        : readEnumInt32(startData, "Type");
                    let ended: boolean | null = null;
                    try {
                        const endData = manager.field<Il2Cpp.Object>("_endData").value;
                        ended = endData !== null && !endData.handle.isNull();
                    } catch {
                        ended = null;
                    }
                    const modeName = mode === null ? null
                        : mode.class.namespace + "." + mode.class.name;
                    candidates.push({ handle, mode: modeName, battleType, ended,
                        hasInGameAttr: attr !== null });
                    // BattleType.Crusade = 18. StartData.Type is authoritative.
                    if (battleType !== 18 || attr === null) continue;
                    const field = int32Field(attr, "ReRandomSkillNum");
                    let entry = crusadeRerollEntries.get(handle);
                    if (entry === undefined) {
                        entry = { reference: manager.ref(true), original: field.value };
                        crusadeRerollEntries.set(handle, entry);
                    }
                    const before = field.value;
                    field.value = target;
                    if (field.value !== target) throw new Error("write verification failed");
                    if (before !== target) changed++;
                    managers.push({ handle, mode: modeName, battleType, ended, before,
                        after: field.value, original: entry.original });
                } catch (error) {
                    errors.push(handle + ": " + error);
                }
            }
            if (managers.length === 0 && errors.length === 0) {
                throw new Error("no BattleLogicMgr with StartData.Type=Crusade(18); " +
                    "candidates=" + JSON.stringify(candidates));
            }
        } else {
            for (const [handle, entry] of [...crusadeRerollEntries]) {
                try {
                    const manager = entry.reference.target;
                    if (manager === null) throw new Error("pinned manager is unavailable");
                    const attr = objectField(manager, "InGameAttr");
                    if (attr === null) throw new Error("InGameAttr is null");
                    const field = int32Field(attr, "ReRandomSkillNum");
                    const before = field.value;
                    field.value = entry.original;
                    if (field.value !== entry.original) throw new Error("restore verification failed");
                    managers.push({ handle, mode: "CrusadeMode", before,
                        after: field.value, original: entry.original });
                    entry.reference.free();
                    crusadeRerollEntries.delete(handle);
                    if (before !== entry.original) changed++;
                } catch (error) {
                    errors.push(handle + ": " + error);
                }
            }
        ...
        log("creroll " + (enabled ? target : "disabled") + ": changed=" + changed +
            ", tracked=" + crusadeRerollEntries.size + ", errors=" + errors.length);
        return { ok: errors.length === 0, enabled, target: enabled ? target : null,
            changed, trackedManagers: crusadeRerollEntries.size, managers, errors };
    });
}
...
// Enum fields may be returned as a number or as an Il2Cpp.ValueType.
function readEnumInt32(object: Il2Cpp.Object, name: string): number {
    const value = object.field<any>(name).value as any;
    if (typeof value === "number") return value;
    if (value !== null && value !== undefined && typeof value.field === "function") {
        const underlying = value.field("value__").value;
        if (typeof underlying === "number") return underlying;
    }
    throw new Error("cannot decode enum field " + object.class.name + "." + name);
}
...
function exPowerCommand(multiplier: number): Promise<object> {
    if (typeof multiplier !== "number" || !Number.isFinite(multiplier) ||
        multiplier < 1 || multiplier > MAX_EX_POWER_MULTIPLIER) {
        return Promise.resolve({ ok: false,
            error: "expower(multiplier) requires 1 <= multiplier <= " +
                MAX_EX_POWER_MULTIPLIER });
    }

    return runCommand("expower", () => {
        const heroes = collectHeroes();
        if (heroes.length === 0) throw new Error("no player heroes found; enter a battle first");
        ...
        heroes.forEach((hero, index) => {
            try {
                const map = attributeMap(hero);
                if (map === null) throw new Error("AttributeMap is null");
                const before = invokeInt32(map, "get_Item", EX_WEAPON_ENERGY_PERCENT_KEY);
                // The game applies (1 + key151). Multiply that complete factor,
                // rather than merely adding a flat percentage to the raw field.
                const calculated = Math.round(
                    (ATTRIBUTE_PERCENT_SCALE + before) * multiplier -
                    ATTRIBUTE_PERCENT_SCALE
                );
                const after = Math.max(-ATTRIBUTE_PERCENT_SCALE,
                    Math.min(STAT_CAP, calculated));
                if (!Number.isSafeInteger(after)) throw new Error("calculated value is not a safe Int32");
                if (after !== before) {
                    runtimeInvokeRaw(map, "set_Item",
                        [EX_WEAPON_ENERGY_PERCENT_KEY, after]);
                    const verified = invokeInt32(map, "get_Item",
                        EX_WEAPON_ENERGY_PERCENT_KEY);
                    if (verified !== after) throw new Error("write verification failed");
                    changed++;
                }
                results.push({ hero: index, handle: hero.handle.toString(),
                    beforeRaw: before, afterRaw: after, multiplier });
            } catch (error) {
                errors.push("Hero " + index + ": " + String(error));
            }
        });

        log("EX-weapon energy x" + multiplier + ": heroes=" + heroes.length +
            ", changed=" + changed + ", errors=" + errors.length);
        return { ok: errors.length === 0, multiplier,
            key: EX_WEAPON_ENERGY_PERCENT_KEY, changed, heroes: results, errors };
    });
}
...
setImmediate(() => {
    log("ready: revive(), reroll(n), creroll(), creroll(n), creroll(false), inc(percent), heal(percent), expower(multiplier), read(), god(), god(seconds), god(false), traceend(), traceend(false), traceendstatus(); startup commands are supplied by hook_pid.json");
});
