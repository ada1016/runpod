...
const BATTLE_LOGIC_MGR_NAME = "HotFix.Battle.BattleLogicMgr";
const GAME_ATTRIBUTE_GROUP_NAME = "HotFix.Battle.GameAttributeGroup";
const AD_REVIVE_TIMES_FIELD_NAME = "AdReviveTimes";
...
function resolveAdReviveClasses(): boolean {
    if (battleLogicMgrClass !== null && gameAttributeGroupClass !== null) {
        return true;
    }
    ...
    try {
        battleLogicMgrClass = assembly.image.class(BATTLE_LOGIC_MGR_NAME);
        gameAttributeGroupClass = assembly.image.class(
            GAME_ATTRIBUTE_GROUP_NAME
        );
        return true;
    } catch (_) {
...
function setAdReviveTimesOnce(value: number): AdReviveResult {
    const target = Math.max(0, Math.min(1000, Math.trunc(value)));
    if (!resolveAdReviveClasses() ||
        battleLogicMgrClass === null ||
        gameAttributeGroupClass === null) {
        return {
            resolved: false,
            ...
            error:
                ...
                BATTLE_LOGIC_MGR_NAME + " or " +
                GAME_ATTRIBUTE_GROUP_NAME
        };
    }

    const field = findFieldInHierarchy(
        gameAttributeGroupClass,
        AD_REVIVE_TIMES_FIELD_NAME
    );
    if (field === null) {
        return {
            resolved: true,
    ...
    if (fieldType !== "System.Int32") {
        return {
            ...
            entries: [],
            error:
                "refusing non-Int32 write to " +
                AD_REVIVE_TIMES_FIELD_NAME
        };
    }

    try {
        const managers = Il2Cpp.gc.choose(battleLogicMgrClass);
        const entries: AdReviveEntry[] = [];
        let changed = 0;
        ...
        for (const manager of managers) {
            ...
            const inGameAttr = objectField(manager, "InGameAttr");
            if (inGameAttr === null) {
                continue;
            }

            const address = inGameAttr.handle.add(field.offset);
            const before = address.readS32();
            if (before !== target) {
                address.writeS32(target);
                changed++;
            }

            entries.push({
                logicManager: manager.handle.toString(),
                inGameAttr: inGameAttr.handle.toString(),
                before,
                after: address.readS32()
            });
        }
...
function setGodModeCommand(enabled: boolean): object {
...
function setRerollCommand(value: number): object {
    if (!Number.isFinite(value)) {
        throw new Error("reroll(value) requires a finite number");
    }

    settings.extraRerollEnabled = true;
...
function setExtraRerollCommand(
    enabled: boolean,
    value?: number
): object {
...
function setAdReviveTimesCommand(value = 5): Promise<object> {
    return new Promise(resolve => {
        Il2Cpp.perform(() => {
            if (tickBusy) {
                resolve({
...
const shortcutGlobals = globalThis as ShortcutGlobals;
shortcutGlobals.inc = incrementAttackCommand;
shortcutGlobals.god = setGodModeCommand;
shortcutGlobals.reroll = setRerollCommand;
shortcutGlobals.renew = setAdReviveTimesCommand;
...
rpc.exports = {
    ...
    setgodmode(enabled: boolean): object {
        return setGodModeCommand(enabled);
    },

    inc_att(enabled: boolean, value?: number): object {
        return setAttackCommand(enabled, value);
    },

    setextrareroll(enabled: boolean, value?: number): object {
        return setExtraRerollCommand(enabled, value);
    },
    ...
    setadrevivetimes(value?: number): Promise<object> {
        return setAdReviveTimesCommand(
            typeof value === "number" ? value : 5
        );
    },

    adrevivestatus(): object {
        return {
            classesResolved:
                battleLogicMgrClass !== null &&
                gameAttributeGroupClass !== null,
