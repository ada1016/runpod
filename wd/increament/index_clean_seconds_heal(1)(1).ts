 ...
 * god(false), miss(), miss(25), miss(false), traceend(), traceend(false),
 * traceendstatus().
 ...
 * miss() adds 100 percentage points to each current player hero's
 * ExtraEvasionChance; miss(percent) selects another percentage and miss(false)
 * restores the exact values saved when each hero was first enrolled.
 * miss mode is a one-shot AttributeMap mutation: no method hook or polling.
...
const ROLE_ATTRIBUTE_TYPE = "HotFix.Battle.RoleAttributeType";
...
const missEntries = new Map<string, MissEntry>();
let missPercent = 0;
...
function roleAttributeKey(name: string): number {
    const klass = findClass(BATTLE_ASSEMBLY, ROLE_ATTRIBUTE_TYPE);
    if (klass === null || !klass.isEnum) {
        throw new Error(ROLE_ATTRIBUTE_TYPE + " enum is not loaded");
    }
    const field = klass.tryField<Il2Cpp.ValueType>(name);
    if (field === null || !field.isStatic || !field.isLiteral ||
        field.type.name !== ROLE_ATTRIBUTE_TYPE) {
        throw new Error("enum member not found: " + ROLE_ATTRIBUTE_TYPE + "." + name);
    }
    // frida-il2cpp-bridge represents a static enum constant as a ValueType;
    // its string form is the member name, not the underlying dictionary key.
    const enumValue = field.value;
    const value = enumValue.field<number>("value__").value;
    if (!Number.isSafeInteger(value) || value < 0 || value > 0x7fffffff) {
        throw new Error("invalid enum value for " + name + ": " + value);
    }
    return value;
}

function missCommand(value: boolean | number = 100): Promise<object> {
    if (typeof value !== "boolean" &&
        (typeof value !== "number" || !Number.isFinite(value) ||
            value <= 0 || value > MAX_MISS_PERCENT)) {
        return Promise.resolve({ ok: false,
            error: "use miss(), miss(false), or miss(percent) with 0 < percent <= 100" });
    }
    const enabled = value !== false;
    const requestedPercent = typeof value === "number" ? value : 100;
    const rawIncrease = Math.round(requestedPercent * ATTRIBUTE_PERCENT_SCALE / 100);

    return runCommand("miss", () => {
        const key = roleAttributeKey("ExtraEvasionChance");
        const errors: string[] = [];
        let changed = 0;
        ...
        if (enabled) {
            ...
            for (const hero of heroes) {
                ...
                let entry = missEntries.get(handle);
                try {
                    const map = attributeMap(hero);
                    if (map === null) throw new Error("AttributeMap is null");
                    if (entry === undefined) {
                        const original = invokeInt32(map, "get_Item", key);
                        entry = { reference: hero.ref(true), original, applied: original };
                        missEntries.set(handle, entry);
                    }
                    const after = Math.min(STAT_CAP, entry.original + rawIncrease);
            ...
            if (errors.length === 0) missPercent = requestedPercent;
        } else {
            for (const [handle, entry] of [...missEntries]) {
                try {
                    const hero = entry.reference.target;
                    if (hero === null) throw new Error("pinned hero is unavailable");
                    const map = attributeMap(hero);
                    if (map === null) throw new Error("AttributeMap is null");
                    runtimeInvokeRaw(map, "set_Item", [key, entry.original]);
                    if (invokeInt32(map, "get_Item", key) !== entry.original) {
                        throw new Error("restore verification failed");
                    }
                    entry.reference.free();
                    missEntries.delete(handle);
                    changed++;
                } catch (error) {
                    errors.push(handle + ": " + error);
                }
            }
            if (errors.length === 0) missPercent = 0;
        }

        log("miss " + (enabled ? requestedPercent + "%" : "disabled") +
            ": changed=" + changed + ", tracked=" + missEntries.size +
            ", errors=" + errors.length);
        return { ok: errors.length === 0, enabled, percent: enabled ? requestedPercent : 0,
            attribute: "ExtraEvasionChance", key, rawIncrease: enabled ? rawIncrease : 0,
            changed, trackedHeroes: missEntries.size, errors };
    });
}
...
setImmediate(() => {
    log("ready: revive(), reroll(96), inc(n), heal(percent), read(), god(), god(seconds), god(false), miss(), miss(percent), miss(false), traceend(), traceend(false), traceendstatus(); automatic reroll(96) scheduled");
